...
- **Resume semantics** are still "transcript file exists and is non-empty → skip." Re-running the same command is the supported resume mechanism. In project mode, `_plan_sources` first lists every URL via `downloader.list_sources` and asks every question up front (YouTube channel roots → pick tabs; each tab/playlist/profile → its own "how many" prompt and its own project folder; plain video URLs share one folder), then `_process_project` runs each unattended, and files are named `utils.video_stem` → `"<title> [<id>]"`, so resume is keyed on the video id rather than list position (channels are newest-first, so positional names would shift on every upload). Already-done videos are filtered before the loop; no-speech videos get a `[no speech detected]` placeholder so re-syncs don't retry them. `_process_project` stops the whole run after `STOP_AFTER_SAME_FAILURES` (5) consecutive identical run-wide download errors (stale cookies, bot checks); per-video errors (`_PER_VIDEO_ERRORS`: age-restricted, private…) never count. Errors print via `short_error` (one line + a hint from `_ERROR_HINTS`).
